...
import { isWorkerMode } from "@nawatt-works/mypi-runtime-mode";

const CHECK_INTERVAL_MS = 24 * 60 * 60 * 1000;
const CHECK_TIMEOUT_MS = 10_000;

function findAggregateRoot(start: string): string {
	...
	for (;;) {
		...
		if (existsSync(packagePath)) {
			...
			try {
				...
				if (Array.isArray(value.workspaces) && value.workspaces.includes("capabilities/global/*")) return directory;
			} catch {
				// Keep walking; npm will report a malformed aggregate package if no valid root is found.
			}
		}
		const parent = dirname(directory);
		if (parent === directory) return packageRoot ?? resolve(start);
		directory = parent;
	}
}

const SETUP_ROOT = findAggregateRoot(dirname(fileURLToPath(import.meta.url)));
