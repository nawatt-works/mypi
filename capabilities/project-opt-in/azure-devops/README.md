...
> **Status:** stable · **Scope:** project opt-in

Project-configured Azure DevOps Boards and Repos capability for Pi. Packageนี้เป็นเจ้าของ extension, configuration/security contract, testsและคู่มือนี้ Root stable manifestไม่โหลด packageโดยอัตโนมัติ แต่ละ project ต้องอ้าง `capabilities/project-opt-in/azure-devops/` ผ่าน `.pi/settings.json` และผ่าน project trust ก่อน Pi จึงจะ execute extension
...
{
  "packages": [
    "/Users/developer/my-project/my-pi/capabilities/project-opt-in/azure-devops"
  ]
}
```
