...
`/mypi-worker-acceptance` เป็น Development/incubator-only harnessที่ clone pinned source, วิ่ง patched leader → generated Worker → real-provider artifact → stop/replacement/cleanup และ appendเฉพาะ redacted evidence; harnessผ่าน independent reviewแล้ว แต่ actual runยัง exit `78`จน operatorรัน `/mypi-worker-setup setup`ใน Development Pi profile Production activationยัง disabled
