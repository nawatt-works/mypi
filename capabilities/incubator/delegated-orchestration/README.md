...
`/mypi-worker-acceptance` เป็น Development/incubator-only harnessที่ clone pinned source, ใช้ exact acceptance dependency lock, วิ่ง patched leader → generated Worker → real-provider artifact → stop/replacement/cleanup และ appendเฉพาะ redacted evidence Operator setupและ generated-path runผ่าน 7/7 checksแล้วโดย observed interactive requests `0`; production activationยัง disabledจน crash/retry, rotationและ final Phase 2–3 reviewครบ
